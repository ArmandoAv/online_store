...
def create_sequence():
    try:
        cursor.execute("CREATE SEQUENCE orderdet_id_seq START WITH 1 INCREMENT BY 1")
        conn.commit()
        print("orderdet_id_seq successfully created in SQL Server.")
...
def drop_sequence():
    try:
        ...
        conn.commit()
        print("orderdet_id_seq sequence successfully droped in SQL Server.")
        ...
        for ord in range(max_order_ids):
            ...
            for det in range(num_details):
                ...
                cursor.execute("""
                    ...
                    VALUES (NEXT VALUE FOR orderdet_id_seq, ?, ?, ?, ?)
